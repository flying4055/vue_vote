	...
	created() {
		...
		if (!this.$store.state.token) {
			...
			if (ua.match(/MicroMessenger/i) == "micromessenger") {
				// 跳转到微信授权页面
				// http%3a%2f%2fvote.crjblog.cn%2findex%2findex%2findex
				let redirect_uri = Cookies.get("beforeLoginUrl", fullPath);
				window.location.href =
					"https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx581ac1620490998c&redirect_uri=" +
					redirect_uri +
