    ...
    <div class="gem-box">
      ...
      <div class="gem-btn-group">
        <div class="gem-btn-item" v-for="item in pay_list" :key="item.id">
          <div
            class="gem-btn"
            v-bind:class="{ btnActive: item.isBtnActive }"
            @click="onClickBtn(item)"
          >
            <div class="gem-img">
              <img :src="item.img_url" alt="" />
            </div>
            <div>{{ item.title }}({{ item.vote_num }})赞</div>
      ...
      <div class="tips" v-if="currentPrice">
        <p>
          价格 {{ currentPrice.price }} 元, 增加{{ currentPrice.vote_num }}赞
        </p>
      </div>
      <van-button type="primary" size="large" @click="clickPayBtn()"
        >微信支付</van-button
      >
      <!-- <p>2元 = 10赞 5元 = 28赞 10元 = 58赞 50元 = 288赞 100元 = 588赞 200元 = 1288赞</p> -->
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 2元10赞   5元28赞    10元58赞    50元288赞    100元588赞     200元1288赞
